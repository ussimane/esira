 ...
 * @author user
 */
public class PlanoCurricularController2 extends GenericForwardComposer {
    ...
    private Window windowFaculdade, mDialogAddFaculdade, mDialogAddACient, windowACient, mDialogAddCurso, windowCurso,
            mDialogAddDiscip, windowDiscip, winDiscDetalhe, windowEquivalencia, winEquivDialog;
    ...
    private Combobox cbdirector, cbfaculdade, cbPeriodDisc, cbcaracter, cbareacient, cbPlanoP, cbPlanoP2, cbCursoPrec, cbCursoPrec2, cbdiscipPre, cbSemPrec, cbprec;
    private Intbox fid, aid, cid, intSemestre, intDiscNivel, intcredito, did, txtPlano, txtPlanoD, ibPlanoP, ibCursoP;
...
//    //............................................................................................................
    ...
    public ListModel<Curso> getCurso2Model() {
        // return cursoModel;
        Map<String, Object> par = new HashMap<>();
        par.put("idf", usr.getUtilizador());
        Faculdade f = ((Users) csimp.findByJPQuery("from Users u where u.utilizador = :idf", par).get(0)).getFaculdade();
...
//    }
    ...
    public void onChange$cbCursoPrec2(Event event) {
        if (cbCursoPrec2.getSelectedItem() != null) {
            Curso c = (Curso) cbCursoPrec2.getSelectedItem().getValue();
            par.clear();
            par.put("c", c);
            List<Planocurricular> lc = csimp.findByJPQuery("from Planocurricular p where p.curso = :c order by p.planocurricularPK.ano desc", par);
            cbPlanoP2.setModel(new ListModelList<Planocurricular>(lc));
            cbPlanoP2.setText(c.getPlanoc() + "");
        }
        selCursoPrec();
    }
    ...
    public void onSelect$cbPlanoP2() {
        selCursoPrec();
    }
...
//    } 
    public void selCursoPrec() {
        if (cbCursoPrec.getSelectedItem() == null||cbCursoPrec2.getSelectedItem() == null) {
            return;
        }
        Curso c = (Curso) cbCursoPrec.getSelectedItem().getValue();
        Map<String, Object> par = new HashMap<String, Object>();
...
//                                Disciplina d = (Disciplina) litem.getValue();
////                                    Equivalencia p = new Equivalencia();
////                                    p.setIdDisc(d.getIdDisc());
////                                    p.setDisciplina(d);
////                                    p.setEquivalencia((Disciplina) cb.getSelectedItem().getValue());
//                                Map<String, Object> par = new HashMap<String, Object>();
//                                par.put("d", d.getIdDisc());
//                                Equivalencia pre = (Equivalencia) csimp.findByJPQuery("from Equivalencia p where p.idDisc = :d", par).get(0);
//                                csimp.delete(pre);
//                                Clients.showNotification(" Apagado com sucesso", null, null, null, 2000);
//                                //selCursoPrec();
...
//    }
    public void onAddPrec(ForwardEvent evt) {
        ...
        Map<String, Object> par = new HashMap<String, Object>();
        par.put("cu", c);
        if (cbPlanoP2.getSelectedItem() != null) {
            par.put("planoc", ((Planocurricular) cbPlanoP2.getSelectedItem().getValue()).getPlanocurricularPK().getAno());
        } else {
            par.put("planoc", c.getPlanoc());// ent.put("d", Disciplina.class);
        }
        String sql = "from Disciplina d where d.curso = :cu and d.planoc = :planoc";
        ldis = csimp.findByJPQuery(sql, par);
        ldis.remove(d);
...
////                                    if (p.getEquivalencia().getIdDisc() != null) {
//                                    csimp.update(p);
...
///////////////////////////////////// precedencia ///////////////////////////////////
    public void onOpen$cbprec() {
        if (cbprec.getModel() == null) {
            ...
            Curso c = csimp.get(Curso.class, ibCursoP.getValue().longValue());
            par.put("cu", c);
    ...
    public void onBtnva() {
        ...
        if (!((ListModelList) lbprec.getListModel()).contains(d)) {
    ...
    public void onBtnRegistar() {
        Disciplina d = csimp.get(Disciplina.class, iddisc.getValue().longValue());
        Iterator<Equivalencia> items = new ArrayList(d.getEquivalenciaList1()).listIterator();
        Equivalencia p;
        while (items.hasNext()) {
            p = items.next();
            d.getEquivalenciaList1().remove(p);
            csimp.delete(csimp.get(Equivalencia.class, p.getEquivalenciaPK()));
        }
        Equivalencia pe;
        Iterator<Listitem> items1 = lbprec.getItems().iterator();
        while (items1.hasNext()) {
            Disciplina di = (Disciplina) items1.next().getValue();
            pe = new Equivalencia(new EquivalenciaPK(d.getIdDisc(), di.getIdDisc()));
            pe.setDisciplina(di);
            pe.setDisciplina1(d);
            d.getEquivalenciaList1().add(pe);
        }
        csimp.update(d);
    ...
    public void onEditPrecDisc(ForwardEvent evt) {
        ...
        Disciplina d = (Disciplina) litem.getValue();
        d = csimp.load(Disciplina.class, d.getIdDisc());
        Window w = (Window) Executions.createComponents("/planoc/formEquiva.zul", windowEquivalencia, null);
        ((Intbox) w.getFellow("iddisc")).setValue(d.getIdDisc().intValue());
        ((Intbox) w.getFellow("ibitem")).setValue(litem.getIndex());
        Curso c = (Curso) cbCursoPrec2.getSelectedItem().getValue();
        if (cbPlanoP2.getSelectedItem() != null) {
            ((Intbox) w.getFellow("ibPlanoP")).setValue(((Planocurricular) cbPlanoP2.getSelectedItem().getValue()).getPlanocurricularPK().getAno());
        } else {
            ((Intbox) w.getFellow("ibPlanoP")).setValue(c.getPlanoc());// ent.put("d", Disciplina.class);
        }
        // ((Intbox) w.getFellow("ibPlanoP")).setValue(ibPlanoP2.getValue());
        Intbox ibc = ((Intbox) w.getFellow("ibCursoP"));
        if (ibc.getValue() != null&&ibc.getValue().longValue()!=c.getIdCurso().longValue()) {
            Combobox cbp=((Combobox) w.getFellow("cbprec"));
            cbp.setModel(null);
            ibc.setValue(c.getIdCurso().intValue());
        } else {
            ibc.setValue(c.getIdCurso().intValue());
        }
        ((Textbox) w.getFellow("lbNome")).setValue(d.getNome());
        Listbox docs = ((Listbox) w.getFellow("lbprec"));
        int sl = d.getEquivalenciaList1().size();
        if (sl > 0) {
            ((Listbox) w.getFellow("lbprec")).setRows(sl + 1);
        }
        Long inc =c.getIdCurso();
        List<Equivalencia> li = d.getEquivalenciaList1();
        final Iterator<Equivalencia> items = li.iterator();
        Disciplina i;
        List<Disciplina> ld = new ArrayList<Disciplina>();
        while (items.hasNext()) {
            i = items.next().getDisciplina();
            if(i.getCurso().getIdCurso()==inc)ld.add(i);
        }
