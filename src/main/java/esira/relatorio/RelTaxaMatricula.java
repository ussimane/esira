...
import entidade.EstudanteMatricula;
 ...
 * @author Tafula
 */
public class RelTaxaMatricula extends GenericForwardComposer {
...
//            totale.setValue("Total: " + l);
            ...
            lbEstudantes.setModel(new ListModelList<Prescricao>());
             Long l = csimpm.countJPQuery("select count(e.idEstudante) from Estudante e where 1=1 " + condnome + condnr + condfac + " and e.idEstudante in (select m.estudante from Matricula m where "
                + "m.matriculaPK.ano = :anol)" + condanoi + condnivel
                + condcurso + ord, condpar);
            totali.setValue("Total Matriculados: " + l);
            Float f = csimpm.sumJPQuery("select sum(a.valor) from Arquivomatricula a where a.matricula.matriculaPK.ano = :anol and a.recibousado is not null and a.recibousado is false and a.matricula.estudante in (Select e from "
                    + "Estudante e where 1=1 " + condfac + condanoi + condnivel
                    + condcurso+")", condpar);
            totale.setValue(" Total de Propina: " + f);

        }
        List<Estudante> li = csimpm.findByJPQueryFilter("from Estudante e where 1=1 " + condnome + condnr + condfac + " and e.idEstudante in (select m.estudante from Matricula m where "
                + "m.matriculaPK.ano = :anol)" + condanoi + condnivel
                + condcurso + ord, condpar, i, j);
         ListModelList lm = new ListModelList<>();
        List<EstudanteMatricula> lde = new ArrayList<EstudanteMatricula>();
        ...
        while (items.hasNext()) {
            e = items.next();
            EstudanteMatricula de = new EstudanteMatricula();
            de.setEstudante(e);
            par.replace("e", e);
            List<Arquivomatricula> ai = csimpm.findByJPQuery("from Arquivomatricula a where a.matricula.matriculaPK.ano = :anol "
                    + "and a.matricula.estudante = :e", par);
            de.setAi(ai);
          //  lde.add(de);
