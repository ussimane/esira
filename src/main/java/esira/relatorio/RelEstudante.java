 ...
 * @author Tafula
 */
public class RelEstudante extends GenericForwardComposer {
    ...
    private Intbox ibPAno, ibAno, ibNivel;
    ...
    private Label lb, totale;
    private Button btv;
    static String ord = "";
    private List<Estudante> estudantes = null;
    String condfac = "", condnr = "", condnome = "", condgenero = "", condanoi = "", condnac = "", condprov = "",
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        cal.setTime(dano);
        txProcurar.setStyle("display:none");
        txPNrmec.setStyle("display:none");
        cbprovincia.setStyle("display:none");
        pgenero.setStyle("display:none");
        cbnacionalidade.setStyle("display:none");
...
//    
//    public void onClick$mactivo(Event e) throws InterruptedException {//por defeito mostra-se todos estudantes activos e nao activos
...
//   }
    ...
    public void onClickplfoto(ForwardEvent evt) {
        setLB(0, 20);
    }
...
//    }
    public void setLB(int i, int j) {
        if (j == 20) {
            lbEstudantes.setModel(new ListModelList<Prescricao>());//modellist de prescricao??
            Long l = csimpm.countJPQuery("select count(e.idEstudante) from Estudante e where 1=1" + condnome + condnr + condfac + condgenero + condanoi + condnivel + condmatric
                   +condminsc + condnac + condprov + condactivo + condcurso + ord, condpar);
            totale.setValue("Total: " + l);

        }// (k.estudante.graduado is null or k.estudante.graduado is false)
        List<Estudante> li = csimpm.findByJPQueryFilter("from Estudante e where 1=1" + condnome + condnr + condfac + condgenero + condanoi + condnivel + condmatric
               +condminsc + condnac + condprov + condactivo + condcurso + ord, condpar, i, j);
