 ...
 * @author ESIRA
 */
public class GuardarEstudanteController extends GenericForwardComposer {
    ...
    private Intbox ibidEstudante, litem, idl, tipov, ibplanoc, ibturno;
    ...
    private Combobox cbviaIngresso, cbnaturalidade, cbTurno;
    ...
    private Combobox cbprovinciapr, cbprovinciaEndAL, cbprovinciaEndPF;
    //Endereco1
...
//        }
    ...
    public void onError$cbTurno() {
        Clients.scrollIntoView(cbTurno);
    }
...
//                }
            ...
            estudante.setPlanoc(ibplanoc.getValue());
            estudante.setTurno(ibturno.getValue());
            estudante.setTurma(1);
        }
...
//                limpar(tbimg4);
            ...
            if ((mDialogMatricula.getTitle().charAt(0) == 'R')) {
                ...
                csimpm.update(mat);
                if (!usr.getUestudante()) {
//Aqui se tem em conta que uma matricula apenas pode ser editada pelo funcionario apartir da lista de admissao para umma matricula
...
//                    ((ListModelList) lbox.getModel()).set(litem.getValue(), la);
                    ...
                    Clients.showNotification(" Matricula Renovada com Sucesso", null, null, null, 2000);
...
//                                .getItemAtIndex(litem.getValue()));
        ...
        estudante = csimpm.get(Estudante.class, estudante.getIdEstudante());
        Curso curs = estudante.getCursocurrente();
        curs = csimpm.get(Curso.class, curs.getIdCurso());
        int idfac = curs.getFaculdade().getIdFaculdade();
        if (mat.getModoMatricula() != 0) {
            String tipopedido = "";
            if (mat.getModoMatricula() == 1) {
                tipopedido = "f4";
            } else {
                tipopedido = "f9";
            }
            ValidacaopendentePK vpk = new ValidacaopendentePK(idfac, tipopedido);
            ...
            if (vp != null) {
                ...
                vp.setFaculdade(curs.getFaculdade());
                vp.setQtd(1);
                csimpm.Save(vp);
            }
            eq = EventQueues.lookup("valid" + idfac, EventQueues.APPLICATION, true);
            eq.publish(new Event("onPedidoMatD", null, vp));
        }
        eq = EventQueues.lookup("mat" + idfac, EventQueues.APPLICATION, true);
        eq.publish(new Event("onPedidoMat", null, mat));
        eq = EventQueues.lookup("mat" + estudante.getIdEstudante(), EventQueues.APPLICATION, true);
        eq.publish(new Event("onPedidoMat", null, mat));
        if (la != null) {
            la = csimpm.get(Listaadmissao.class, la.getIdaluno());
            eq = EventQueues.lookup("uladm" + idfac, EventQueues.APPLICATION, true);
            eq.publish(new Event("onPedidoMat", null, la));
        }
        if (mat.getModoMatricula() != 0) {
            eq = EventQueues.lookup("pmatD" + idfac, EventQueues.APPLICATION, true);
    ...
    public void onAceitarPedido() {
        Messagebox.show("Pretende validar este Pedido de Matricula?", "", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,
                new EventListener() {
                    @Override
...
//                                }
                                ...
                                mat.setEstado(true);
                                final Estudante estudante = mat.getEstudante();
                                ...
                                csimpm.update(mat);
                                verificarMatAnul(mat);
                                String s = "";
                                boolean b = false;
                                final Obj ei = new Obj();
                                ei.setId(0);
                                if (estudante.getUsersList().isEmpty()) {
                                    Users user = new Users();
                                    user.setNome(estudante.getNomeCompleto());
                                    user.setUtilizador(estudante.getNomeCompleto().charAt(0) + estudante.getApelido());
                                    user.setUtilizador(semAcento(user.getUtilizador().toLowerCase()));
                                ...
                                if (estudante.getListaadmissaoList() != null && !estudante.getListaadmissaoList().isEmpty()) {
                                    Listaadmissao la = estudante.getListaadmissaoList().get(0);
...
//                                new Listbox().appendChild(((Listbox) mDialogMatricula.getParent().getFellow("lbEstudantes"))
//                                        .getItemAtIndex(litem.getValue()));
                                ///////////
                                if (mat.getModoMatricula() != 0) {
                                    String tipopedido = "";
                                                    ...
                                                    switch (((Integer) evet.getData()).intValue()) {
                                                        case Messagebox.YES:
                                                            ...
                                                            Clients.evalJavaScript("var xhttp = new XMLHttpRequest();\n"
                                                                    ...
                                                                    + "xhttp.open('POST', 'EnviarEmailRecibo?tipo=1&id1=" + estudante.getIdEstudante().intValue()
                                                                    + "&id2=" + mat.getMatriculaPK().getAno() + "&idf=" + f.getIdFuncionario().intValue() + "&ido=" + ei.getId() + "', true);\n"
                                                                    + " xhttp.send();");
                                                            break;
                                                        case Messagebox.NO:
                                                    }
                                ...
                                || (estudante.getMudancac() != null && estudante.getMudancac().intValue() == 0))) {
                                    ...
                                    final int modo = mat.getModoMatricula();
                                    Messagebox.show("Pretende Validar as Inscricao do: " + estudante.getNomeCompleto(), "", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,
                                            new EventListener() {
                                                @Override
                                                public void onEvent(Event evet) throws InterruptedException {
                                                    switch (((Integer) evet.getData()).intValue()) {
                                                        case Messagebox.YES:
                                                            Window win;
                                                            if (modo > 1) {
                                                                win = (Window) Executions.createComponents("/inscricao/PedidoInscricao2.zul", mDialogMatricula, null);
                                                                ((Textbox) win.getFellow("txx")).setText(id + "");
                                                                win.setTitle("Validacao de Inscricao");
                                                                win.setClosable(true);
                                                                win.doModal();
                                                            } else {
                                                                win = (Window) Executions.createComponents("/inscricao/PedidoInscricao.zul", mDialogMatricula, null);
                                                                win.setTitle("Validacao de Inscricao");
                                                                win.setClosable(true);
                                                                ((Textbox) win.getFellow("txx")).setText(id + "");
                                                                win.doModal();
    ...
    public void verificarMatAnul(Matricula mat) {
        par.clear();
        par.put("e", mat.getMatriculaPK().getIdEstudante());
        List<Matriculaanulada> manul = csimpm.findByJPQuery("from Matriculaanulada ma where ma.matricula.estudante.idEstudante = :e and ma.renovada is false", par);
        Matriculaanulada ma;
        Iterator<Matriculaanulada> itemsi2 = manul.iterator();
    ...
    public void onRejeitarPedido() {
        Messagebox.show("Pretende Rejeitar este Pedido de Matricula?", "", Messagebox.YES | Messagebox.NO, Messagebox.QUESTION,
                new EventListener() {
                    @Override
                        ...
                        switch (((Integer) evet.getData()).intValue()) {
                            case Messagebox.YES:
                                ...
                                if (tipov.getValue() == 2) {
                                    ...
                                    mm.setMotivo(txMotivoR.getText());
                                                ...
                                                public void onEvent(Event evet) throws InterruptedException, IOException, EmailException, DocumentException {
                                                    switch (((Integer) evet.getData()).intValue()) {
                                                        case Messagebox.YES:
                                                            ...
                                                            Clients.evalJavaScript("var xhttp = new XMLHttpRequest();\n"
                                                                    + "xhttp = new XMLHttpRequest();"
                                                                    + "xhttp.open('POST', 'EnviarEmailRecibo?tipo=2&id1=" + mat.getEstudante().getIdEstudante().intValue()
                                                                    + "&id2=" + mat.getMatriculaPK().getAno() + "&idf=" + f.getIdFuncionario().intValue() + "&ido=" + 0 + "', true);\n"
                                                                    + " xhttp.send();");
                                                            break;
                                                        case Messagebox.NO:
                                                    }
...
//        Users u = csimpm.findEntByJPQuery("from Users u where u.utilizador = :ut", par);
    ...
    public void onVerInfo(ForwardEvent evt) throws JRException, IOException {
        ...
        String path = mDialogMatricula.getDesktop().getWebApp().getRealPath("/relatorio");
        String path2 = mDialogMatricula.getDesktop().getWebApp().getRealPath("/img");
